...
(function () {
  ...
  /* ── the chart ───────────────────────────────────────────────────────── */
  function chart(into) {
    var hs = R.hours, n = hs.length, iNow = R.iNow, nar = narrow();
    var first = FULL ? 0 : Math.max(0, iNow - (nar ? 24 : 72));
    var W = nar ? 420 : 900, H = nar ? 200 : (FULL ? 260 : 220), m = { t: 24, r: 8, b: 22, l: 30 };
    ...
    R.days.forEach(function (dy) { if (dy.past) return; var mid = dy.hours[Math.floor(dy.hours.length / 2)].i; if (mid < first + 4) return; svg.appendChild(txt(el("text", { class: "tick fc-daylab", x: x(mid), y: m.t - 8, "text-anchor": "middle" }), Math.round(dy.mean))); });
    ...
    svg.appendChild(path(function (q) { return q.w; }, first, iNow, { stroke: "var(--accent)", "stroke-width": 1.5, "stroke-opacity": ".55" }));
    svg.appendChild(path(function (q) { return q.obs; }, first, iNow, { stroke: "var(--ink)", "stroke-width": 2 }));
    svg.appendChild(path(function (q) { return q.fc; }, iNow, n - 1, { stroke: "var(--accent)", "stroke-width": 2.5 }));
