  ...
  function tiles(into) {
    ...
    R.days.forEach(function (dy, k) {
      ...
      tile.innerHTML = "<div class='fc-dow'>" + (dy.today ? "Today" : DAY[d.getUTCDay()]) + "</div>" +
        ...
        // The indicators the model leans on: 100 m wind, the daily mean lid, rain.
        // The glyph points down and is rotated by the direction the wind comes
        // FROM, so a north wind points south — where it blows — and the letter
        // names its origin, as a wind rose does.
        "<div class='fc-wx'>" +
          "<span class='fc-wind' title='Wind at 100 m, daily mean'><svg viewBox='0 0 12 12' width='11' height='11' aria-hidden='true' style='transform:rotate(" + Math.round(dy.dir) + "deg)'>" +
