...
(function () {
  ...
  var CROSSES = [];
  function hideTip() { tip.style.opacity = 0; CROSSES.forEach(function (c) { if (c.isConnected) c.setAttribute("opacity", 0); }); CROSSES = CROSSES.filter(function (c) { return c.isConnected; }); }
  ...
  function chart(into, opts) {
    ...
    if (opts.hours) {
      ...
      for (var i = first; i < n; i++) {
        var d = hs[i].d; if (d.getUTCHours() !== 0 || i === first) continue;
        // A hairline at every midnight, so a day's start and end can be read off the chart.
        svg.appendChild(el("line", { x1: x(i), x2: x(i), y1: m.t, y2: m.t + ph, stroke: "var(--ink-faint)", "stroke-opacity": ".55", "stroke-width": .8 }));
        if (n - i >= 12) svg.appendChild(txt(el("text", { class: "tick", x: x(i) + 3, y: H - 6 }), DAY[d.getUTCDay()] + (FULL && !nar ? " " + d.getUTCDate() : "")));
      }
    ...
    svg.querySelector(".scrub").addEventListener("pointerleave", function (ev) { if (ev.pointerType !== "touch") cross.setAttribute("opacity", 0); });
    CROSSES.push(cross);
  }
