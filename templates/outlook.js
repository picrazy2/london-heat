...
(function (root, factory) {
  ...
  function nextDate(s) { var d = dateOf(s + "T00"); d.setUTCDate(d.getUTCDate() + 1); return d.toISOString().slice(0, 10); }
  ...
  function outlook(PM25, model, meta, inp) {
    ...
    days.forEach(function (d) {
      ...
      // The evening: 20:00 to 02:00 the next morning, because the dusk build-up
      // peaks after midnight as often as before it.
      var eve = hs.filter(function (h) { var hh = h.d.getUTCHours(); return hh >= 20 && ok(h.v); });
      var nextDay = byDay[nextDate(d.date)];
      if (nextDay) nextDay.hours.forEach(function (h) { if (h.d.getUTCHours() <= 2 && ok(h.v)) eve.push(h); });
